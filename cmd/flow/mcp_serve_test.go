...
func TestMCPServer_HappyPath_AirtableE2E(t *testing.T) {
...
`
	...
	// 8. getRun (optional: just check run exists)
	getRunParams := struct{ RunID string }{rid.RunID}
	resp, err = client.CallTool(ctx, "getRun", getRunParams)
	if err != nil {
		t.Fatalf("getRun failed: %v", err)
	...
	if resp == nil || len(resp.Content) == 0 {
		return // Allow run to be nil if completed
	}
	if resp.Content[0] == nil {
		return // Allow run to be nil if completed
	}
